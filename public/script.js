...
function restoreState() {
  ...
  if (savedState) {
    try {
      ...
      const savedPerProject = parseInt(
        localStorage.getItem(`lastQuestion_p${currentProject}`)
      );
      if (!Number.isNaN(savedPerProject) && savedPerProject > 0) {
        currentQuestion = savedPerProject;
      } else {
        currentQuestion =
          state.currentQuestion || getFirstQuestionId(currentProject);
      }
...
function updateQuestionStatus() {
  const needsReview =
    localStorage.getItem(`review_p${currentProject}_q${currentQuestion}`) ===
    "true";
  const isCompleted =
    localStorage.getItem(`completed_p${currentProject}_q${currentQuestion}`) ===
    "true";
        ...
        <div class="question-status">
            <button class="status-btn review-btn ${
              needsReview ? "active" : ""
                ...
                あとで見直す <span class="check-mark">${
                  needsReview ? "✓" : " "
                }</span>
            </button>
            <button class="status-btn answered-btn ${
              isCompleted ? "active" : ""
            }" 
                    onclick="toggleAnsweredStatus(${currentProject}, ${currentQuestion})">
                回答済み <span class="check-mark">${
                  isCompleted ? "✓" : " "
                }</span>
            </button>
...
function getSavedOrFirstQuestionId(projectNumber) {
  const saved = parseInt(
    localStorage.getItem(`lastQuestion_p${projectNumber}`)
  );
  ...
  projectData.forEach((question, index) => {
    ...
    const isCompleted =
      localStorage.getItem(
        `completed_p${currentProject}_q${question.questionId}`
      ) === "true";
    ...
    const needsReview =
      localStorage.getItem(
        `review_p${currentProject}_q${question.questionId}`
      ) === "true";
...
function updateTimerDisplay() {
  ...
  const timerElement = document.getElementById("timer");
  if (timerElement) {
    timerElement.textContent = `残り時間: ${minutes
      .toString()
...
function isProjectCompleted(projectNumber) {
  ...
  return projectData.every((question) => {
    return (
      localStorage.getItem(
        `completed_p${projectNumber}_q${question.questionId}`
      ) === "true"
    );
  });
}
...
function initializeReviewPage() {
  ...
  for (let p = 1; p <= maxProjects; p++) {
    ...
    projectData.forEach((question) => {
      const needsReview =
        localStorage.getItem(`review_p${p}_q${question.questionId}`) === "true";
      const isCompleted =
        localStorage.getItem(`completed_p${p}_q${question.questionId}`) ===
        "true";
      ...
      row.innerHTML = `
                <td class="question-cell">
                    <a href="#" class="question-link" onclick="goToQuestion(${p}, ${
        question.questionId
      })">
                ...
                <td class="status-cell">
                    ${
                      needsReview
                        ? '<span class="status-label review">要レビュー</span>'
                        : ""
                    }
                </td>
                <td class="status-cell">
                    ${
                      isCompleted
                        ? '<span class="status-label completed">回答済み</span>'
                        : ""
                    }
                </td>
            `;
...
function updateQuestionDisplay() {
  ...
  // 現在の問題を取得
  const currentQuestionData = projectData.find(
...
function toggleReviewStatus(project, question) {
  const currentStatus =
    localStorage.getItem(`review_p${project}_q${question}`) === "true";
...
function toggleAnsweredStatus(project, question) {
  const currentStatus =
    localStorage.getItem(`completed_p${project}_q${question}`) === "true";
...
document.addEventListener("keydown", (event) => {
  ...
  // Alt + C: あとで見直すボタンの切り替え
  if (event.altKey && event.key === "c") {
    event.preventDefault();
    toggleReviewStatus(currentProject, currentQuestion);
  }

  // Alt + D: 回答済みボタンの切り替え
  if (event.altKey && event.key === "d") {
    event.preventDefault();
    toggleAnsweredStatus(currentProject, currentQuestion);
  }

  // Alt + V: 解説動画を開く
  if (event.altKey && event.key === "v") {
    event.preventDefault();
    openVideoExplanation();
  }

  // Alt + Shift + V: 解説動画（CMなし）を開く
  if (event.altKey && event.shiftKey && event.key === "V") {
    event.preventDefault();
    openVideoExplanationNoCm();
  }
...
function getAvailableProjects() {
  const appData = questionData[selectedApp] || {};
  return Object.keys(appData).filter(
    (key) => key.startsWith("project") && Array.isArray(appData[key])
  ).length;
}
